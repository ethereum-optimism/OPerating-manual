...
Version validation is a strict, critical requirement. To securely hand over upgradability to the Collective, a chain’s L1 smart contracts must be deployed by the canonical OP Contracts Manager ("OPCM") address on Ethereum, [`todo: add opcm contract address`](). This is an automated L1 deployment contract which deploys chains based on `op-contracts@v7.0.0`.
